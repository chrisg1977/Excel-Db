...
type Queryable = {
  query: (sql: string, params?: any[]) => Promise<QueryResultLike>;
};
 ...
 * Formula: 
 *   Full-timer (40 hrs/week): hourly_rate × 40 × 52/12
 *   Part-timer: hourly_rate × contracted_hours_per_week × 52/12
 * No adjustments for working days - flat calculation per Excel payroll formula
 */
export const calculateBasicWage = (
  ...
  // Direct calculation: contracted_hours × hourly_rate × (52/12)
  // For 40-hour weeks: 40 × rate × 4.33333 = monthly wage
  // For part-timers: contracted_hours × rate × 4.33333
  const basicWage = contractedHoursPerWeek * hourlyRate * WEEKS_PER_MONTH;
