...
const ACTIVE_SHIFT_STATUSES = ['open', 'report_in_progress'] as const;
...
type SupportedRole = 'operational' | 'supervisor' | 'management' | 'admin';
...
type CreateShiftSessionPayload = {
  ...
  previous_cashbox_end: number | null;
  corrected_opening_cash: number | null;
  manager_unavailable: boolean;
};

type TakeoverShiftSessionPayload = {
  takeover_reason: string;
};

type AbandonShiftSessionPayload = {
  abandon_reason: string;
};

type SupersedeShiftSessionPayload = CreateShiftSessionPayload & {
...
type ShiftSessionRow = {
  ...
  taken_over_from_user: string | null;
...
type ShiftSessionAuditPayload = {
  action: 'manager_takeover' | 'shift_abandoned' | 'shift_superseded';
  ...
  previous_shift_session_id: string | null;
  new_shift_session_id: string | null;
};

type ShiftSessionApiResult = {
  status: number;
...
const normalizeRequiredFreeText = (value: unknown, key: string): string => {
  if (typeof value !== 'string' || !value.trim()) {
    throw new Error(`${key} is required`);
  }
  return value.trim();
...
const normalizeShiftDate = (value: unknown): string => {
...
const normalizeShiftTime = (value: unknown): string => {
...
const normalizeBoolean = (value: unknown, key: string): boolean => {
...
const normalizeNonNegativeNumber = (value: unknown, key: string): number => {
...
const parseCreateShiftSessionPayload = (payload: Record<string, unknown>): CreateShiftSessionPayload => {
...
const parseRequestBody = (req: Request): CreateShiftSessionPayload => {
  return parseCreateShiftSessionPayload(getRequestBodyRecord(req));
};

const parseTakeoverRequestBody = (req: Request): TakeoverShiftSessionPayload => {
...
const parseAbandonRequestBody = (req: Request): AbandonShiftSessionPayload => {
  const payload = getRequestBodyRecord(req);
  return {
...
const parseSupersedeRequestBody = (req: Request): SupersedeShiftSessionPayload => {
  const payload = getRequestBodyRecord(req);
  return {
...
const normalizeRole = (value: string | undefined): SupportedRole => {
...
const mapShiftSessionRow = (row: Record<string, unknown>): ShiftSessionRow => ({
  id: String(row.id),
  ...
  previous_cashbox_end:
    row.previous_cashbox_end === null || row.previous_cashbox_end === undefined
      ? null
      : Number(row.previous_cashbox_end),
  corrected_opening_cash:
    row.corrected_opening_cash === null || row.corrected_opening_cash === undefined
      ? null
  ...
  taken_over_from_user:
    row.taken_over_from_user === null || row.taken_over_from_user === undefined
      ? null
      : String(row.taken_over_from_user),
  taken_over_at:
    row.taken_over_at === null || row.taken_over_at === undefined
      ? null
      : new Date(String(row.taken_over_at)).toISOString(),
  takeover_reason:
    row.takeover_reason === null || row.takeover_reason === undefined
      ? null
      : String(row.takeover_reason),
  status: String(row.status ?? '')
});

const toShiftSessionSummary = (row: ShiftSessionRow): ShiftSessionSummary => ({
  id: row.id,
...
const assertManagerOrHigher = (user: CurrentUserContext) => {
  if (!user.isManagerOrHigher) {
    throw new Error('Manager or admin access is required');
  }
};

const isActiveShiftStatus = (status: string): boolean => {
  return ACTIVE_SHIFT_STATUSES.includes(status as (typeof ACTIVE_SHIFT_STATUSES)[number]);
};

const isUnresolvedShiftStatus = (status: string): boolean => {
  return UNRESOLVED_SHIFT_STATUSES.includes(status as (typeof UNRESOLVED_SHIFT_STATUSES)[number]);
};

const normalizeRequiredSessionIdParam = (req: Request): string => {
...
const findLatestShiftByLocationAndStatuses = async (
  client: PoolClient,
  locationCode: string,
  ...
  return mapShiftSessionRow(result.rows[0] as Record<string, unknown>);
};

const findDuplicateShiftSession = async (
  client: PoolClient,
  payload: CreateShiftSessionPayload
): Promise<ShiftSessionRow | null> => {
  const result = await client.query(
    `SELECT
       id::text AS id,
     ...
     ORDER BY created_at DESC
     LIMIT 1`,
    [payload.clinic_code, payload.shift_date, payload.shift_start_time]
  );
...
const insertShiftSession = async (
  client: PoolClient,
  payload: CreateShiftSessionPayload,
  user: CurrentUserContext,
  options: { supersedesShiftSessionId?: string | null } = {}
): Promise<ShiftSessionRow> => {
  const result = await client.query(
    `INSERT INTO eos_shift_session (
       id,
       location_code,
       department_code,
       clinic_code,
...
const findShiftSessionByIdForUpdate = async (
  client: PoolClient,
...
const buildCreateSuccessResponse = (row: ShiftSessionRow): ShiftSessionApiResult => ({
  status: 201,
  body: {
    ok: true,
    id: row.id,
...
const normalizeIsoDateTime = (value: Date | string): string => {
  const parsed = value instanceof Date ? new Date(value.getTime()) : new Date(value);
  return parsed.toISOString();
};

const buildShiftSessionAuditPayload = (options: {
  action: ShiftSessionAuditPayload['action'];
  shiftSession: ShiftSessionRow;
  previousOwner: string;
  newOwner: string;
  actedBy: string;
  actedAt: string | null;
  reason: string;
  previousShiftSessionId?: string | null;
  newShiftSessionId?: string | null;
}): ShiftSessionAuditPayload => ({
...
const buildTakeoverAuditPayload = (
  row: ShiftSessionRow,
  previousOwner: string,
  actingUser: CurrentUserContext,
  takeoverReason: string
): ShiftSessionAuditPayload =>
  buildShiftSessionAuditPayload({
...
const buildTakeoverSuccessResponse = (
  row: ShiftSessionRow,
  audit: ShiftSessionAuditPayload
): ShiftSessionApiResult => ({
  status: 200,
  body: {
    ok: true,
...
const buildAbandonSuccessResponse = (
  row: ShiftSessionRow,
  audit: ShiftSessionAuditPayload
): ShiftSessionApiResult => ({
  status: 200,
...
const buildSupersedeSuccessResponse = (
  row: ShiftSessionRow,
  audit: ShiftSessionAuditPayload
): ShiftSessionApiResult => ({
  status: 201,
...
const buildShiftSessionNotFoundResponse = (): ShiftSessionApiResult => ({
  status: 404,
  body: {
    ok: false,
...
const buildShiftSessionNotActiveResponse = (row: ShiftSessionRow): ShiftSessionApiResult => ({
  status: 409,
  body: {
    ok: false,
...
const buildShiftSessionNotUnresolvedResponse = (row: ShiftSessionRow): ShiftSessionApiResult => ({
  status: 409,
  body: {
    ok: false,
...
const buildActiveShiftAlreadyExistsResponse = (row: ShiftSessionRow): ShiftSessionApiResult => ({
  status: 409,
  body: {
    ok: false,
...
const updateShiftSessionTakeover = async (
  client: PoolClient,
  row: ShiftSessionRow,
  actingUser: CurrentUserContext,
  takeoverReason: string
  ...
  const result = await client.query(
    `UPDATE eos_shift_session
     ...
     RETURNING
       ...
       takeover_reason,
       status`,
    [row.id, actingUser.username, previousOwner, takeoverReason]
  );

  return mapShiftSessionRow(result.rows[0] as Record<string, unknown>);
};

const updateShiftSessionAbandon = async (
  client: PoolClient,
  row: ShiftSessionRow,
  actingUser: CurrentUserContext,
  ...
  const result = await client.query(
    `UPDATE eos_shift_session
     ...
     RETURNING
       ...
       takeover_reason,
       status`,
    [row.id, actingUser.username, abandonReason]
  );

  return mapShiftSessionRow(result.rows[0] as Record<string, unknown>);
};

const updateShiftSessionSuperseded = async (
  client: PoolClient,
  previousShiftSessionId: string,
  newShiftSessionId: string,
  actingUser: CurrentUserContext
): Promise<void> => {
  await client.query(
    `UPDATE eos_shift_session
     ...
     WHERE id::text = $1`,
    [previousShiftSessionId, newShiftSessionId, actingUser.username]
  );
};

const buildSameOwnerResumeResponse = (row: ShiftSessionRow): ShiftSessionApiResult => ({
  status: 200,
  body: {
    ok: true,
...
const buildBlockedByOtherUserResponse = (row: ShiftSessionRow): ShiftSessionApiResult => ({
  status: 409,
  body: {
    ok: false,
...
const buildManagerTakeoverAvailableResponse = (row: ShiftSessionRow): ShiftSessionApiResult => ({
  status: 409,
  body: {
    ok: false,
...
const buildUnresolvedShiftResponse = (
  row: ShiftSessionRow,
  user: CurrentUserContext
): ShiftSessionApiResult => {
  if (!user.isManagerOrHigher) {
...
const buildEmergencyHandoverAvailableResponse = (row: ShiftSessionRow): ShiftSessionApiResult => ({
  status: 409,
  body: {
    ok: false,
...
const buildDuplicateShiftSessionResponse = (): ShiftSessionApiResult => ({
  status: 409,
  body: {
    ok: false,
...
export const createOrResumeShiftSession = async (
  pg: PgPool,
  req: Request
): Promise<ShiftSessionApiResult> => {
  ...
  try {
    ...
    const unresolvedShift = await findLatestShiftByLocationAndStatuses(
      client,
      payload.location_code,
      UNRESOLVED_SHIFT_STATUSES
    );
    if (unresolvedShift) {
      await client.query('ROLLBACK');
...
export const takeOverShiftSession = async (pg: PgPool, req: Request): Promise<ShiftSessionApiResult> => {
  ...
  try {
    ...
    if (!ACTIVE_SHIFT_STATUSES.includes(existingShiftSession.status as (typeof ACTIVE_SHIFT_STATUSES)[number])) {
      await client.query('ROLLBACK');
      return buildShiftSessionNotActiveResponse(existingShiftSession);
    }

    const updatedShiftSession = await updateShiftSessionTakeover(
      client,
      existingShiftSession,
      actingUser,
      takeoverPayload.takeover_reason
    );
    await client.query('COMMIT');

    // TODO: Persist this audit payload in a dedicated EOS audit store once
    // shift-session audit persistence is implemented.
    const audit = buildTakeoverAuditPayload(
      updatedShiftSession,
      existingShiftSession.current_owner,
      actingUser,
      takeoverPayload.takeover_reason
    );
...
export const abandonShiftSession = async (pg: PgPool, req: Request): Promise<ShiftSessionApiResult> => {
  const actingUser = resolveCurrentUserContext(req);
  assertManagerOrHigher(actingUser);
  ...
  try {
    ...
    const updatedShiftSession = await updateShiftSessionAbandon(
      client,
      existingShiftSession,
      actingUser,
      abandonPayload.abandon_reason
    );
    await client.query('COMMIT');
...
export const supersedeShiftSession = async (pg: PgPool, req: Request): Promise<ShiftSessionApiResult> => {
  const actingUser = resolveCurrentUserContext(req);
  assertManagerOrHigher(actingUser);
  ...
  try {
    ...
    const createdShiftSession = await insertShiftSession(
      client,
      supersedePayload,
      actingUser,
      { supersedesShiftSessionId: existingShiftSession.id }
    );
...
export const handleCreateShiftSessionRequest = async (pg: PgPool, req: Request, res: Response) => {
  try {
...
export const handleTakeoverShiftSessionRequest = async (pg: PgPool, req: Request, res: Response) => {
  try {
    // TODO: Replace placeholder header-based auth/role lookup with the real EOS auth/session source.
...
export const handleAbandonShiftSessionRequest = async (pg: PgPool, req: Request, res: Response) => {
  try {
    // TODO: Replace placeholder header-based auth/role lookup with the real EOS auth/session source.
...
export const handleSupersedeShiftSessionRequest = async (pg: PgPool, req: Request, res: Response) => {
  try {
    // TODO: Replace placeholder header-based auth/role lookup with the real EOS auth/session source.
