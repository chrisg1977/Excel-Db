export type RateTier = {
  up_to_eur: number | null; // null => no upper limit
  percent: number;
};

export type PercentageRule =
  | { mode: 'FLAT'; percent: number }
  | { mode: 'TIERED'; tiers: RateTier[] };

export type ProviderDuesInput = {
  provider_production_total: number;
  lab_fees_total: number;
  production_rule: PercentageRule;
  lab_rule: PercentageRule;
  is_subscribed_main_payroll: boolean;
  payslip_value?: number; // required when main payroll=true
  official_bank_transfer_input?: number; // used for non-main flow
  bank_fees?: number; // deducted from cash side in both flows
  cash_rounding_unit?: number; // default 10 EUR
  cash_rounding_mode?: 'UP' | 'NEAREST'; // non-main flow only
};

export type ProviderDuesResult = {
  production_due: number;
  lab_due: number;
  net_due_before_settlement: number;
  official_amount: number;
  cash_amount: number;
  bank_transfer_amount: number;
  bank_fees_deducted_from_cash: number;
  notes: string[];
};
...
const computeByRule = (amount: number, rule: PercentageRule): number => {
  if (rule.mode === 'FLAT') {
...
const roundCash = (value: number, unit: number, mode: 'UP' | 'NEAREST') => {
  if (value <= 0) return 0;
  if (mode === 'UP') return Math.ceil(value / unit) * unit;
  return Math.round(value / unit) * unit;
};

export const calculateProviderDues = (input: ProviderDuesInput): ProviderDuesResult => {
