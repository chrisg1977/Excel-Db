...
export type ProductionVisitsRequest = {
  clinic_code?: string;
  clinic_num?: number;
...
export type ProductionVisitRow = {
  time: string;
...
const getSingleQueryValue = (value: unknown, key: string): QueryValue => {
...
const normalizeRequiredText = (value: QueryValue, key: string): string => {
  if (!value) throw new Error(`${key} is required`);
  return value;
};

const normalizeDepartmentCode = (value: QueryValue): string => {
  return normalizeRequiredText(value, 'department_code').toUpperCase();
};

const normalizeClinicCode = (value: QueryValue): string | undefined => {
  if (!value) return undefined;
  return value.toUpperCase();
};

const normalizeClinicNum = (value: QueryValue): number | undefined => {
  if (!value) return undefined;
...
const normalizeMysqlDateTime = (value: QueryValue, key: string): string => {
...
const parseRequest = (req: Request): ProductionVisitsRequest => {
  ...
  const reportStartAt = normalizeMysqlDateTime(
    getSingleQueryValue(req.query.report_start_at, 'report_start_at'),
    'report_start_at'
  );
  const reportEndAt = normalizeMysqlDateTime(
    getSingleQueryValue(req.query.report_end_at, 'report_end_at'),
    'report_end_at'
  );
...
const resolveClinic = async (mysql: MysqlPool, filters: ProductionVisitsRequest): Promise<ClinicResolution> => {
  if (filters.clinic_num && filters.clinic_code) {
    const [rows] = await mysql.query(
      `SELECT ClinicNum, Abbr
       FROM clinic
       WHERE ClinicNum = ? AND Abbr = ?
       LIMIT 1`,
      [filters.clinic_num, filters.clinic_code]
    );
    const matches = rows as Array<{ ClinicNum: number; Abbr: string }>;
    if (!matches.length) {
...
const loadProductionVisits = async (
  mysql: MysqlPool,
  filters: ProductionVisitsRequest,
  clinic: ClinicResolution
): Promise<ProductionVisitRow[]> => {
  ...
  const [rows] = await mysql.query(
    `SELECT
     ...
     ORDER BY grouped.appointment_datetime, grouped.patient_number`,
    [filters.department_code, filters.report_start_at, filters.report_end_at, clinic.clinic_num]
  );
...
export const getProductionVisits = async (
  mysql: MysqlPool,
  req: Request
): Promise<{ rows: ProductionVisitRow[]; filters: ProductionVisitsRequest & ClinicResolution }> => {
...
export const handleProductionVisitsRequest = async (mysql: MysqlPool, req: Request, res: Response) => {
  try {
