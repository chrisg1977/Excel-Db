...
export type SocialSecurityContribution = {
  class_code: string;
...
const ageOnYearEnd = (dob: string | null, year: number): number | null => {
  if (!dob) return null;
...
export const lookupSocialSecurityContribution = async (
  pg: PgPool,
  year: number,
  weeklyWage: number,
  dateOfBirth: string | null
): Promise<SocialSecurityContribution | null> => {
  if (weeklyWage <= 0) return null;
  ...
  const result = await pg.query(
    `SELECT
     ...
     LIMIT 1`,
    [year, weeklyWage, dateOfBirth, age]
  );

  const row = result.rows[0];
  if (!row) return null;

  const employeeContribution = computeContributionAmount(
    weeklyWage,
    row.employee_fixed === null ? null : Number(row.employee_fixed),
    row.employee_percentage === null ? null : Number(row.employee_percentage),
    null
  );
  const employerContribution = computeContributionAmount(
    weeklyWage,
    row.employer_fixed === null ? null : Number(row.employer_fixed),
    row.employer_percentage === null ? null : Number(row.employer_percentage),
    null
  );
  const mlfContribution = computeContributionAmount(
    weeklyWage,
    row.mlf_fixed === null ? null : Number(row.mlf_fixed),
    row.mlf_percentage === null ? null : Number(row.mlf_percentage),
    row.mlf_max === null ? null : Number(row.mlf_max)
  );
