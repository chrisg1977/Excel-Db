...
import {
  calculateTax,
  determineSocialSecurityClass,
  lookupSocialSecurityContribution,
  lookupTaxBracket
} from './wage-calculator.js';

type PayrollKind = 'MAIN' | 'PROVIDER' | 'THIRDPARTY';

type PayrollRow = {
  emp_id: number;
  surname: string;
  first_name: string;
  provider_subscription_exists: boolean;
  designation: string;
  department: string;
  ...
  period_from: string;
  period_to: string;
  timesheet_acquired: string;
  last_timesheet_import: string | null;
  hourly_rate: number;
  contracted_weekly_hours: number;
  net_payment: number;
  gross_including_bonuses: number;
  gross_total: number;
  my_cost_per_cheque: number;
  vl_taken_ytd: number;
  vl_taken_period: number;
  vl_pending: number;
  vl_warning: string;
  sl_taken_ytd: number;
  sl_taken_period: number;
  sl_pending: number;
  sl_warning: string;
  banked_hours_balance: number;
  warnings: string[];
  hours_worked: number;
  ...
  ss_employer_contribution: number;
...
type PayrollRateSource = {
  ...
  notes: string;
  source_emp_id: number | null;
};

type PayrollMonthResultRow = {
  id: number;
  employment_type_main: string;
  weekly_hours_main: number;
  pay_input_basis_main: string;
  input_amount_main: number;
  hourly_rate_main: number;
  tax_status: string;
  student_flag: boolean;
  timesheet_required: boolean | null;
  annual_vl_entitlement_hours: number;
  annual_sl_entitlement_hours: number;
  gross_total: number;
  normal_main_income: number;
  taxable_bonus_total: number;
  performance_bonus: number;
  supervisor_bonus: number;
  bonus: number;
  statutory_bonus_amount: number;
  normal_rate_extra_pay: number;
  overtime_pay_amount: number;
  unpaid_leave_hours_total: number;
  unpaid_leave_deduction_amount: number;
  main_base_monthly_wage: number;
  tax_on_gross: number;
  ssc_weekly_wage_basis: number;
  ssc_employee: number;
  ssc_employer: number;
  ssc_total: number;
  mlf: number;
  ...
  payslip_value: number;
  banked_hours_opening: number;
  banked_hours_movement: number;
  banked_hours_closing: number;
  vl_taken_hours: number;
  sl_taken_hours: number;
  calc_status: string;
  source_funds_label: string;
  transaction_number: string;
  paid_previously_amount: number;
  calc_notes: string;
  review_reason: string;
};

type PayslipSaveResult = {
  payslipId: number;
...
type ExcelYearViewRow = {
  ...
  extra_under_hours: number | null;
  extra_due: number | null;
  bonus: number | null;
  performance_bonus: number | null;
  supervisor_bonus: number | null;
  bonus_total: number | null;
  tax: number | null;
  overtime_hours: number | null;
  overtime_tax: number | null;
  ssc_employer: number | null;
  ssc_employee: number | null;
  ssc_total: number | null;
  mlf: number | null;
  other_amount: number | null;
  net_wage: number | null;
  leave_balance_end: number | null;
  banked_hours_since_last_month: number | null;
...
const PAYROLL_ALIAS: Record<string, PayrollKind> = {
  MAIN: 'MAIN',
  PROV: 'PROVIDER',
  PROVIDER: 'PROVIDER',
  O3P: 'THIRDPARTY',
...
const parsePayroll = (value: unknown): PayrollKind => {
...
const truncateMoney = (value: number) => {
...
const cleanIban = (value: unknown) => {
  const text = cleanText(value).replace(/\s+/g, '');
  if (!text) return '';
  if (/^(0|#N\/A|null|undefined)$/i.test(text)) return '';
  return text;
};

const parseHoursText = (value: unknown) => {
...
const countMondaysInMonth = (year: number, month: number) => {
  ...
  while (date.getMonth() === (month - 1)) {
    if (date.getDay() === 1) mondays += 1;
    date.setDate(date.getDate() + 1);
  }
  return mondays;
};

const normalizeLegacyPayType = (value: string) => {
  const raw = cleanText(value).toUpperCase();
  if (raw === 'HOURLY_RATE') return 'HOURLY';
  if (raw === 'MONTHLY_RATE') return 'MONTHLY';
  if (raw === 'YEARLY_RATE') return 'YEARLY';
  return raw;
};

const getCurrentMonthResult = async (
  pg: PgPool,
  payroll: PayrollKind,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  empId: number
): Promise<PayrollMonthResultRow | null> => {
  const result = await pg.query(
    `SELECT *
     ...
     LIMIT 1`,
    [empId, payroll, period.year, period.month]
  );
  return (result.rows[0] as PayrollMonthResultRow | undefined) || null;
};

const upsertMainHourlyRateOverride = async (
  pg: PgPool,
  empId: number,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  hourlyRate: number
  ...
  const employment = await pg.query(
    `SELECT
...
type ExcelPayslipMonth = {
  ...
  basic_wage: number | null;
  extra_under_label: string;
  extra_under_hours_text: string;
  extra_under_amount: number | null;
  ssc_total: number | null;
  ssc_employer: number | null;
  ssc_employee: number | null;
  payer_pe_text: string;
  unpaid_leave_hours_text: string;
  unpaid_leave_amount: number | null;
  mlf: number | null;
  bonus: number | null;
  overtime_tax: number | null;
  other_amount: number | null;
  tax_on_gross: number | null;
  performance_bonus: number | null;
  supervisor_bonus: number | null;
  iban: string;
  ...
  net_wage: number | null;
  leave_taken_to_date: number | null;
  transaction_id_text: string;
  weeks_value: number | null;
  banked_hours_since_last_month: number | null;
  leave_balance_end: number | null;
  clinic_addresses: string;
  rows: Array<{ row_key: string; cells: unknown[] }>;
};

type ExcelPayslipEmployee = {
  sheet: string;
  months: Record<string, ExcelPayslipMonth>;
};

let excelPayslipCachePromise: Promise<Record<string, ExcelPayslipEmployee>> | null = null;
...
const loadExcelPayslipCache = async () => {
  if (!excelPayslipCachePromise) {
    excelPayslipCachePromise = fs
      ...
      .then((raw) => {
        const parsed = JSON.parse(raw) as { employees?: Record<string, ExcelPayslipEmployee> };
...
const getExcelPayslipMonth = async (empId: number, periodRaw: string) => {
  const cache = await loadExcelPayslipCache();
  return cache[String(empId)]?.months?.[periodRaw] || null;
};

const getExcelSourceEmpId = async (pg: PgPool, empId: number) => {
  const result = await pg.query(
    `SELECT source_emp_id
     FROM employees
     WHERE emp_id = $1
     LIMIT 1`,
    [empId]
...
const getExcelPayslipMonthWithFallback = async (pg: PgPool, empId: number, periodRaw: string) => {
...
const getExcelPayslipYearView = async (pg: PgPool, payroll: PayrollKind, empId: number, periodRaw: string) => {
  const cache = await loadExcelPayslipCache();
  ...
  if (!employee?.months) {
    return { rows: [] as ExcelYearViewRow[], previousYearSummary: null as null | Record<string, unknown> };
  }
  ...
  const payrollStatuses = await pg.query(
    `SELECT
     ...
     ORDER BY pe.payroll_month, ps.id DESC`,
    [empId, `${payroll}-%`, year]
  );
  const statusByMonth = new Map<string, string>();
  for (const row of payrollStatuses.rows) {
    const key = cleanText(row.period_key);
    if (!key || statusByMonth.has(key)) continue;
    statusByMonth.set(key, cleanText(row.payslip_status).toUpperCase());
  }

  const dbRowsByMonth = new Map<string, PayrollRow | null>();
  await Promise.all(
...
const coveredDayCount = (periodFrom: string, periodTo: string) => {
...
const sanitizeFilePart = (value: string) => value.replace(/[\\/:*?"<>|]/g, '_').replace(/\s+/g, ' ').trim();
const monthShort = (month: number) => MONTH_LABELS[Math.max(0, Math.min(11, month - 1))] || 'Month';
...
const lastFridayOfMonth = (year: number, month: number) => {
...
const buildDisplayName = (row: PayrollRow) => `${cleanText(row.first_name)} ${cleanText(row.surname)}`.trim() || String(row.emp_id);

const normalizeTimesheetHourType = (value: unknown) => {
  ...
  if (raw === 'VL') return 'VACATION_LEAVE';
  if (raw === 'SL') return 'SICK_LEAVE';
  if (raw === 'UL') return 'UNPAID_LEAVE';
  if (raw === 'OT') return 'OVERTIME';
  return 'WORK';
};

const normalizeLeaveStatus = (value: unknown, hourType: string) => {
  const raw = cleanText(value).toUpperCase().replace(/\s+/g, '_');
  if (!raw) {
...
const parseEditableTimesheetRows = (payload: unknown): EditableTimesheetRow[] => {
...
const parseTimesheetCsv = (csvText: string): EditableTimesheetRow[] => {
  const lines = String(csvText || '')
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  const rows: EditableTimesheetRow[] = [];
...
const resolveHourlyRate = (payType: string | null | undefined, amount: number) => {
...
const formatPdfMoney = (value: unknown) => `EUR ${toNumber(value).toFixed(2)}`;
const formatPdfHours = (value: unknown) => `${roundMoney(toNumber(value)).toFixed(2)} HRS`;
const isNonZero = (value: unknown) => Math.abs(toNumber(value)) > 0.00001;
const monthCompactLabel = (period: NonNullable<ReturnType<typeof parsePeriod>>) =>
  `${monthShort(period.month)}-${String(period.year).slice(-2)}`;
const weeksInPayrollMonth = (period: NonNullable<ReturnType<typeof parsePeriod>>) =>
  String(Math.ceil(new Date(Date.UTC(period.year, period.month, 0)).getUTCDate() / 7));

const resolveDepartmentBrandLogoPath = (departmentRaw: unknown) => {
...
const readImageBufferFromDataUri = (value: unknown) => {
...
let uploadedPayrollSignatureBufferPromise: Promise<Buffer | null> | null = null;
const loadUploadedPayrollSignatureBuffer = async (pg: PgPool): Promise<Buffer | null> => {
  ...
  uploadedPayrollSignatureBufferPromise = (async () => {
    try {
      const result = await pg.query(
        `SELECT id, item_key, title, description, file_name, mime_type, file_base64, updated_at
      ...
      const scored = result.rows
        .map((item) => {
          ...
          let score = 0;
          if (/signature|sign/.test(haystack)) score += 10;
          if (/gauci|christian|principal|director/.test(haystack)) score += 6;
          if (/payroll/.test(haystack)) score += 3;
          if (/logo/.test(haystack)) score -= 4;
...
const resolvePayslipSignatureBuffer = async (pg: PgPool, signatureRaw: unknown) => {
  const direct = readImageBufferFromDataUri(signatureRaw);
  if (direct) return direct;
  return loadUploadedPayrollSignatureBuffer(pg);
};

const renderLegacyPayslipPdf = (
...
const renderPayslipPdf = async (
  ...
  const bankedApplicable = !(
    employee?.timesheet_required === false ||
    cleanText((employee as Record<string, unknown>)?.timesheet_mode).toUpperCase() === 'NONE'
  );
  ...
  await writePdf(
    ...
    (doc) => {
      ...
      const drawRoundedBox = (x: number, y: number, w: number, h: number, fillColor: string, strokeColor: string, radius = cardRadius) => {
        doc.save();
        doc.roundedRect(x, y, w, h, radius).fillAndStroke(fillColor, strokeColor);
        doc.restore();
      };

      const drawLabelValue = (x: number, y: number, w: number, h: number, label: string, value: string) => {
      ...
      const fsValue = cleanText(employee.fs_status || (employee as Record<string, unknown>).fs4_status);
      ...
      const earnings = [
        ...
        { label: 'Bonus', hours: '', amount: toNumber((calculation as Record<string, unknown>).bonus_total ?? (calculation as Record<string, unknown>).bonus), visible: isNonZero((calculation as Record<string, unknown>).bonus_total ?? (calculation as Record<string, unknown>).bonus) },
      ...
      const metaCols = [
        ['ID Card', cleanText(employee.id_value || (employee as Record<string, unknown>).national_id) || '-'],
        ['Type', cleanText(employee.employment_type) || '-'],
        ['P.E. No.', cleanText(employee.pe_number) || '-'],
        ['Ref', runRef],
        ['Weeks', weeksInPayrollMonth(period)],
        ['Hrs/Wk', String((calculation as Record<string, unknown>).contracted_weekly_hours ?? '-')],
        ['FS / SSC', fsSsc],
      ...
      const deductionMetrics: Array<[string, string]> = [
      ...
      const lowerItems: Array<{ label: string; value: string; sub?: string }> = [
...
const sendPayslipEmail = async (row: PayrollRow, attachmentPath: string, month: number) => {
  console.warn(
    `[payroll] payslip email suppressed during vetting for emp_id=${row.emp_id} attachment=${attachmentPath} month=${monthShort(month)}`
  );
};

const renderFs3Pdf = async (
  row: PayrollRow,
  year: number,
...
const sendFs3Email = async (row: PayrollRow, year: number, attachmentPath: string) => {
  console.warn(
    `[payroll] FS3 email suppressed during vetting for emp_id=${row.emp_id} attachment=${attachmentPath} year=${year}`
  );
};

const safeTaxCategory = (status: string | null | undefined) => {
...
const buildRows = async (
  ...
  const result = await pg.query(
    `WITH employee_scope AS (
     ...
     ORDER BY e.department, e.surname, e.first_name, e.emp_id`,
    [period.periodFrom, period.end.toISOString().slice(0, 10), payroll, period.periodTo, `${payroll}-${period.raw}`]
  );
  ...
  for (const record of result.rows) {
    ...
    if (gross > 0) {
      if (toNumber((record as any).current_result_tax_on_gross) > 0 || toNumber((record as any).current_result_ssc_employee) > 0 || toNumber((record as any).current_result_ssc_employer) > 0) {
        taxDeduction = roundMoney(toNumber((record as any).current_result_tax_on_gross));
        ssEmployeeContribution = roundMoney(toNumber((record as any).current_result_ssc_employee));
        ssEmployerContribution = roundMoney(toNumber((record as any).current_result_ssc_employer));
        mlfContribution = roundMoney(toNumber((record as any).current_result_mlf));
      } else {
        try {
        ...
        try {
          const employeeDobResult = await pg.query(
            `SELECT to_char(dob, 'YYYY-MM-DD') AS dob FROM employees WHERE emp_id = $1 LIMIT 1`,
            [Number(record.emp_id)]
          );
    ...
    const grossTotal = roundMoney(toNumber((record as any).current_result_gross_total) || gross);
    const totalDeductions = roundMoney(taxDeduction + ssEmployeeContribution);
    const netPayment = roundMoney(toNumber((record as any).current_result_net_wage) || (grossTotal - totalDeductions));
...
const loadRowsByEmployee = async (
  pg: PgPool,
  payroll: PayrollKind,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  employeeIds: number[]
...
const getRateSource = async (
  pg: PgPool,
  empId: number,
  period: NonNullable<ReturnType<typeof parsePeriod>>
): Promise<PayrollRateSource | null> => {
  const newTermsResult = await pg.query(
    `SELECT
...
const getPayrollDashboardDetail = async (
  pg: PgPool,
  payroll: PayrollKind,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  empId: number
  ...
  const employeeResult = await pg.query(
    `SELECT
       ec.emp_id,
  ...
  const payrollSubscriptionsResult = await pg.query(
    `SELECT
       payroll_type,
       active_from,
       active_to
  ...
  const workResult = await pg.query(
    `SELECT
    ...
    [empId, row.period_from, row.period_to]
  );

  const savedLineResult = await pg.query(
    `SELECT
       pl.*,
  ...
  const statutoryBonus = roundMoney(
    toNumber(savedLine.statutory_bonus_june) +
  ...
  const otherPayments = roundMoney(
    toNumber(savedLine.weekly_allowance_march) +
...
const upsertPayrollEntry = async (
  client: PoolClient,
  payroll: PayrollKind,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  rows: PayrollRow[]
  ...
  const existing = await client.query(
    `SELECT id FROM payroll_entries WHERE payroll_number = $1 LIMIT 1`,
    [payrollNumber]
  );
...
const upsertPayrollMonthResult = async (
  client: PoolClient,
  payroll: PayrollKind,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  row: PayrollRow,
  options: {
    ...
    sourceFundsLabel?: string | null;
    paidPreviouslyAmount?: number | null;
    reviewAdjustments?: unknown;
    calcStatus?: 'DRAFT' | 'CONFIRMED' | 'AMENDED';
  }
) => {
  const termsResult = await client.query(
    `SELECT *
     ...
     ORDER BY effective_from DESC, id DESC
     LIMIT 1`,
    [row.emp_id, payroll, period.periodFrom, period.periodTo]
  );
  const profileResult = await client.query(
    `SELECT COALESCE(NULLIF(TRIM(fs4_status), ''), 'Single') AS fs4_status,
            fixed_hours_week,
            timesheet_required
  ...
  const current = await client.query(
    `SELECT id, calc_version
     ...
     ORDER BY calc_version DESC, id DESC
     LIMIT 1`,
    [row.emp_id, payroll, period.year, period.month]
  );
...
const syncPayrollHourSourcesAndDecisions = async (
  client: PoolClient,
  payrollResultId: number,
  payroll: PayrollKind,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  row: PayrollRow,
  detail: any
  ...
  await client.query(
    `DELETE FROM payroll_hour_decisions
     WHERE payroll_result_id = $1`,
    [payrollResultId]
  );
  await client.query(
    `DELETE FROM payroll_hour_deficit_decisions
     WHERE payroll_result_id = $1`,
    [payrollResultId]
  );
  await client.query(
    `DELETE FROM payroll_hour_sources
     WHERE emp_id = $1
       AND payroll_type = $2
       AND period_year = $3
       AND period_month = $4`,
  ...
  const insertSource = async (sourceType: string, direction: 'CREDIT' | 'DEFICIT', hoursValue: number) => {
    const inserted = await client.query(
      `INSERT INTO payroll_hour_sources (
       ...
       RETURNING id`,
      [row.emp_id, payroll, period.year, period.month, sourceType, direction, hoursValue]
    );
    return Number(inserted.rows[0].id);
  };
  ...
  if (extraHours > 0) {
    ...
    if (normalExtraPay > 0) {
      await client.query(
        `INSERT INTO payroll_hour_decisions (
    ...
    await client.query(
      `INSERT INTO payroll_hour_deficit_decisions (
         emp_id, payroll_type, period_year, period_month, payroll_result_id, hour_source_id,
...
const syncPayrollLeaveClassifications = async (
  client: PoolClient,
  payrollResultId: number,
  payroll: PayrollKind,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  ...
  await client.query(
    `DELETE FROM payroll_leave_classifications
     WHERE payroll_result_id = $1`,
    [payrollResultId]
  );

  const leaveRows = await client.query(
    `SELECT work_date, hours, hour_type, COALESCE(leave_status, '') AS leave_status
     FROM timesheets
     WHERE emp_id = $1
       AND work_date >= $2::date
       AND work_date < ($3::date + INTERVAL '1 day')
     ...
     ORDER BY work_date`,
    [row.emp_id, period.periodFrom, period.periodTo]
  );
  ...
  for (const leave of leaveRows.rows) {
    ...
    await client.query(
      `INSERT INTO payroll_leave_classifications (
...
const syncPayrollBonusAccruals = async (
  client: PoolClient,
  payrollResultId: number,
  payroll: PayrollKind,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  ...
  await client.query(
    `DELETE FROM payroll_bonus_accruals
     WHERE payroll_result_id = $1`,
    [payrollResultId]
  );
  ...
  const monthKey = ({
    ...
    9: 'SEPTEMBER',
    12: 'DECEMBER'
  } as Record<number, string | undefined>)[period.month];
  if (!monthKey) return;

  const bonusRules = await client.query(
    `SELECT *
  ...
  if (!bonusRules.rows.length) return;

  const unpaidDaysResult = await client.query(
    `SELECT COUNT(DISTINCT work_date)::int AS unpaid_days
     FROM timesheets
     WHERE emp_id = $1
       ...
       AND (hour_type = 'UNPAID_LEAVE' OR COALESCE(leave_status, '') = 'UNPAID')`,
    [row.emp_id, period.year, monthKey]
  );
  const unpaidDays = Number(unpaidDaysResult.rows[0]?.unpaid_days || 0);

  const employeeDates = await client.query(
    `SELECT
...
const syncPayrollSscZeroWeeks = async (
  client: PoolClient,
  payrollResultId: number,
  payroll: PayrollKind,
  period: NonNullable<ReturnType<typeof parsePeriod>>,
  ...
  await client.query(
    `DELETE FROM payroll_ssc_zero_weeks
     WHERE payroll_result_id = $1`,
    [payrollResultId]
  );
  ...
  for (const wk of weekRows.rows) {
    ...
    await client.query(
      `INSERT INTO payroll_ssc_zero_weeks (
         emp_id, payroll_type, period_year, period_month, payroll_result_id,
...
const upsertPayrollLine = async (
  client: PoolClient,
  payrollEntryId: number,
  row: PayrollRow
) => {
  const existing = await client.query(
    `SELECT id FROM payroll_lines WHERE payroll_entry_id = $1 AND emp_id = $2 LIMIT 1`,
    [payrollEntryId, row.emp_id]
  );
...
const upsertPayslip = async (
  ...
  const paymentDate = lastFridayOfMonth(period.year, period.month);
  const existing = await client.query(
    `SELECT id FROM payslips WHERE payroll_entry_id = $1 AND emp_id = $2 LIMIT 1`,
    [payrollEntryId, row.emp_id]
  );
  ...
  if (existing.rows[0]?.id) {
    ...
    await client.query(
      `UPDATE payslips
       ...
       WHERE id = $1`,
      [payslipId, payrollLineId, payslipNumber, paymentDate, status, bankTransactionNumber ?? null, sourceFundsLabel ?? null, paidPreviouslyAmount ?? null, reviewAdjustments ? JSON.stringify(reviewAdjustments) : null]
    );
  ...
  const inserted = await client.query(
    `INSERT INTO payslips (
     ...
     RETURNING id`,
    [payrollLineId, payrollEntryId, row.emp_id, payslipNumber, paymentDate, status, bankTransactionNumber ?? null, sourceFundsLabel ?? null, paidPreviouslyAmount ?? null, reviewAdjustments ? JSON.stringify(reviewAdjustments) : null]
  );
  return {
...
const updatePayslipDelivery = async (
  pg: PgPool,
  payslipId: number,
  data: { status: string; pdfPath?: string | null; emailedAt?: boolean }
) => {
  await pg.query(
    `UPDATE payslips
     SET status = $2,
     ...
     WHERE id = $1`,
    [payslipId, data.status, data.pdfPath ?? null, !!data.emailedAt]
  );
};

export const handlePayrollDashboardOverviewRequest = async (
  pg: PgPool,
  req: Request,
  res: Response
) => {
...
export const handlePayrollDashboardStatusesRequest = async (
  pg: PgPool,
  req: Request,
  res: Response
) => {
  try {
    ...
    const byEmployee = new Map<number, { status: string; issue_date: string | null; email_sent_at: string | null }>();
    for (const row of result.rows) {
      const empId = Number(row.emp_id);
      if (!Number.isInteger(empId) || byEmployee.has(empId)) continue;
...
export const handlePayrollDashboardDetailRequest = async (
  pg: PgPool,
  req: Request,
  res: Response
) => {
...
export const handlePayrollDashboardTimesheetReviewRequest = async (
  pg: PgPool,
  req: Request,
  res: Response
) => {
  try {
    ...
    const [employeeResult, timesheetResult, conflictResult, eventSummaryResult] = await Promise.all([
      pg.query(
        `SELECT emp_id, first_name, surname, designation, department
      ...
      pg.query(
        `SELECT id, emp_id, work_date, hours, hour_type, leave_status, source, notes
         FROM timesheets
         WHERE emp_id = $1
           AND work_date >= $2::date
           AND work_date <= $3::date
         ORDER BY work_date, id`,
        [empId, periodFrom, periodTo]
      ),
      pg.query(
        `SELECT id, emp_id, work_date, existing_hours, imported_hours, status, source_system, notes, created_at
      ...
      pg.query(
        `SELECT COUNT(*) AS event_count, MAX(updated_at) AS last_import_at
         FROM timesheet_events
         WHERE employee_id = $1
           AND event_datetime >= $2::date
           AND event_datetime < ($3::date + INTERVAL '1 day')`,
        [empId, periodFrom, periodTo]
      )
    ]);
...
export const handlePayrollDashboardTimesheetSaveRequest = async (
  pg: PgPool,
  req: Request,
  res: Response
) => {
...
export const handlePayrollDashboardIssueRequest = async (
  pg: PgPool,
  req: Request,
  res: Response
) => {
  ...
  const employeeIds = Array.isArray(req.body?.employee_ids)
    ? req.body.employee_ids.map((value: unknown) => Number(value)).filter((value: number) => Number.isInteger(value) && value > 0)
    : [];
  ...
  try {
    ...
    try {
      ...
      const savedRows: Array<{ row: PayrollRow; payslip: PayslipSaveResult }> = [];
...
export const handlePayrollDashboardRateUpdateRequest = async (
  pg: PgPool,
  req: Request,
  res: Response
) => {
...
export const handlePayrollFs3EmailRequest = async (
  pg: PgPool,
  req: Request,
  res: Response
) => {
...
export const handlePayrollDashboardExportRequest = async (
  pg: PgPool,
  req: Request,
  res: Response
) => {
