...
const normalizeRequiredUuidPathValue = (value, key) => {
...
const normalizeBoolean = (value, key, defaultValue) => {
    if (value === undefined || value === null) {
...
const normalizeReviewActionType = (value) => {
...
const normalizeReviewOutcomeStatus = (value) => {
...
const parseDiscrepancyEventPayload = (req) => {
    ...
    return {
        ...
        admin_summary_required: normalizeBoolean(payload.admin_summary_required, 'admin_summary_required', true),
        status: normalizeStatus(payload.status)
    };
};
const parseDiscrepancyEventListFilters = (req) => {
    ...
    return {
        ...
        created_at_from: normalizeOptionalDateTime(getSingleQueryValue(query.created_at_from, 'created_at_from'), 'created_at_from'),
        created_at_to: normalizeOptionalDateTime(getSingleQueryValue(query.created_at_to, 'created_at_to'), 'created_at_to'),
...
const parseReviewOutcomePayload = (req) => {
    const payload = getRequestBodyRecord(req);
    return {
...
const getUpdatedDiscrepancyEventStatus = (finalOutcomeStatus) => {
    if (finalOutcomeStatus === 'closed')
        return 'closed';
    if (finalOutcomeStatus === 'pending_follow_up')
        return 'pending_manager_review';
    return 'pending_notification';
};
const parseManagerResolutionPreviewJson = (value) => {
...
const asQueueableManagerResolutionPreview = (value) => {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return null;
    return value;
};
const getPreviewRecipientId = (preview, key) => {
...
const buildQueueInsertPayloads = (eventId, payload) => {
...
const insertDiscrepancyEvent = async (client, payload) => {
    const eventId = randomUUID();
    const previewJson = payload.manager_resolution_preview === null
        ? null
...
const insertNotificationQueueRows = async (client, queueItems) => {
    let insertedCount = 0;
...
const buildDiscrepancyEventListFilterParams = (filters) => [
...
const listDiscrepancyEvents = async (client, filters) => {
...
const countDiscrepancyEvents = async (client, filters) => {
    const filterValues = buildDiscrepancyEventListFilterParams(filters);
    const result = await client.query(`
...
const loadDiscrepancyEventDetailById = async (client, eventId) => {
    const result = await client.query(`
      SELECT *
...
const loadDiscrepancyEventById = async (client, eventId) => {
    const result = await client.query(`
      SELECT
        id,
        status
...
const loadLatestManagerReviewOutcome = async (client, eventId) => {
    const result = await client.query(`
      SELECT
...
const loadDiscrepancyEventQueueRows = async (client, eventId) => {
    const result = await client.query(`
      SELECT
        id::text AS id,
        source_event_id::text AS source_event_id,
...
const insertManagerReviewOutcome = async (client, sourceEventId, reviewedBy, payload) => {
...
const updateDiscrepancyEventStatus = async (client, eventId, status) => {
    const result = await client.query(`
...
export const handleListDiscrepancyEventsRequest = async (pg, req, res) => {
    let client = null;
    try {
...
export const handleGetDiscrepancyEventRequest = async (pg, req, res) => {
    let client = null;
    try {
...
export const handleCreateDiscrepancyEventRequest = async (pg, req, res) => {
    let client = null;
    try {
...
export const handleCreateDiscrepancyEventReviewRequest = async (pg, req, res) => {
    let client = null;
    try {
