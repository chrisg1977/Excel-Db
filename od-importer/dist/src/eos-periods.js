...
const assertManagementAccessIfRolePresent = (req) => {
...
const normalizeOptionalPeriodStatus = (value) => {
    if (value === undefined)
        return null;
    const normalized = value.trim().toLowerCase();
...
const parsePeriodListFilters = (req) => {
...
const parsePeriodSummaryFilters = (req) => {
    const query = req.query;
    return {
...
`;
const warnIfUnknownPeriodStatuses = (items) => {
    const unknownStatuses = Array.from(new Set(items
...
const warnIfUnknownScopedStatuses = (row, periodId) => {
    const anomalies = {};
...
const listPeriods = async (client, filters) => {
    const result = await client.query(PERIOD_LIST_SQL, [
        filters.status,
        filters.start_from,
        filters.start_to,
        filters.limit,
...
const loadPeriodSummary = async (client, accountingPeriodId, filters) => {
    const result = await client.query(PERIOD_SUMMARY_SQL, [
        accountingPeriodId,
...
export const handleListAccountingPeriodsRequest = async (pg, req, res) => {
    let client = null;
    try {
...
export const handleGetAccountingPeriodSummaryRequest = async (pg, req, res) => {
    let client = null;
    try {
        // TODO: Harden period-dashboard access with the final shared auth/access layer.
        assertManagementAccessIfRolePresent(req);
