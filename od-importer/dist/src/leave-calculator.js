 ...
 */
export const hasLeaveEntitlement = (payrollType, employmentType) => {
...
export const calculateLeaveEntitlements = async (pg, input) => {
    ...
    const prorataResult = await pg.query(`SELECT 
      employment_start,
      employment_end,
      days_employed,
      prorata_factor,
    ...
    WHERE emp_id = $1 AND year = $2`, [employee_id, payroll_year]);
    ...
    const usageResult = await pg.query(`SELECT 
    ...
    FROM timesheets
    WHERE emp_id = $1 
      AND EXTRACT(YEAR FROM work_date) = $2`, [employee_id, payroll_year]);
...
export const validateLeaveUsage = (entitlements) => {
    const violations = [];
 ...
 */
export const isPaymentDue = (entitlements) => {
 ...
 */
export const isEligibleForPublicHolidayEntitlements = (ssClass, payrollType) => {
 ...
 */
export const getSicknessBenefitRate = async (pg, payrollYear, taxCategory // sng, mar, mar1, mar2, par, par1, par2
) => {
    if (!taxCategory) {
    ...
    const result = await pg.query(`SELECT daily_rate FROM sickness_benefit_rates
     WHERE benefit_year = $1 AND family_status = $2`, [payrollYear, familyStatus]);
    return result.rows.length > 0 ? parseFloat(result.rows[0].daily_rate) : 17.21;
};
export const calculateSicknessBenefitDeduction = (sicklLeaveHoursUsed, grossWage, benefitRatePerDay) => {
