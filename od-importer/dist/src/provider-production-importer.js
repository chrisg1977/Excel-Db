...
const loadProviderId = async (pg, odProvNum) => {
    const result = await pg.query(`SELECT provider_id
...
const loadOpenDentalProduction = async (mysql, periodStart, periodEnd, odProvNum, clinicNums) => {
    ...
    const clinicFilter = clinicNums && clinicNums.length > 0
        ...
        : '';
    const [rows] = await mysql.query(`SELECT ProcNum, ProcDate, ProcFee, ProvNum, ClinicNum
     FROM procedurelog
     WHERE ProcStatus = 2
       AND ProcDate >= ?
       AND ProcDate < ?
       AND ProvNum = ?${clinicFilter}
     ORDER BY ProcDate ASC, ProcNum ASC`, clinicNums && clinicNums.length > 0
        ? [start, endExclusive, odProvNum, ...clinicNums]
        : [start, endExclusive, odProvNum]);
    return rows;
};
const mapRows = (rows, providerId, periodStart, periodEnd) => rows.map((row) => ({
...
const insertImportLog = async (pg, periodStart, periodEnd, odProvNum, clinicNums, splitByClinic) => {
    const result = await pg.query(`INSERT INTO od_provider_production_import_log
      (period_start, period_end, od_prov_num, clinic_nums, split_by_clinic, status)
     VALUES ($1, $2, $3, $4, $5, 'STARTED')
     RETURNING id`, [periodStart, periodEnd, odProvNum, clinicNums ?? null, splitByClinic]);
    return Number(result.rows[0].id);
};
const updateImportLog = async (pg, id, payload) => {
    await pg.query(`UPDATE od_provider_production_import_log
...
const upsertProviderProduction = async (pg, rows, importBatchId) => {
    let inserted = 0;
    let updated = 0;
    ...
    for (const chunk of chunks) {
        ...
        chunk.forEach((row, idx) => {
            ...
            params.push(importBatchId, row.period_start, row.period_end, row.provider_id, row.od_prov_num, row.proc_date, row.clinic_num, row.source_proc_num, row.proc_fee_gross);
        });
        const result = await pg.query(`INSERT INTO od_provider_production
...
export const runProviderProductionImport = async (pg, mysql, payload) => {
