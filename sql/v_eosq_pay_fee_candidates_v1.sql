...
-- Scope:
--   Includes:
--     - v_eosq_pay_lines_raw_v1
--     - v_eosq_fee_lines_raw_v1
--   Excludes:
--     - SELL
--     - ENTRY
--     - PAY SUMMARY
--
-- Run:
--   psql -h <host> -p <port> -U <user> -d <db> -f sql/v_eosq_pay_fee_candidates_v1.sql

DROP VIEW IF EXISTS v_eosq_pay_fee_candidates_v1;

CREATE VIEW v_eosq_pay_fee_candidates_v1 AS
WITH pay_candidates AS (
  ...
  FROM v_eosq_pay_lines_raw_v1 p
  WHERE p.branch_code = 'EOSQ'
),
fee_candidates AS (
  SELECT
    -- Provenance / identity
    ...
    -- Raw source values
    f.raw_col_4 AS raw_business_date_text,
    f.raw_col_6 AS raw_department_text,
    COALESCE(NULLIF(btrim(f.raw_col_10), ''), NULLIF(btrim(f.raw_col_7), '')) AS raw_category_text,
    f.raw_col_15 AS raw_payment_channel_text,
    f.raw_col_14 AS raw_receipt_text,
    f.raw_col_16 AS raw_ex_vat_text,
    f.raw_col_13 AS raw_vat_text,
    f.raw_col_11 AS raw_total_text,
    f.raw_col_8 AS raw_reference_text,
    f.raw_col_7 AS raw_description_text,
    f.raw_col_5 AS raw_shift_text,
    f.raw_col_8 AS raw_merchant_text,
    NULL::text AS raw_running_capital_text,
    f.raw_col_10 AS raw_type_text,
    f.raw_col_9 AS raw_invoice_text,
    f.raw_col_17 AS raw_assistant_text,
    ...
    -- Candidate values (only where safely derivable)
    f.parsed_business_date AS business_date_candidate,
    NULLIF(btrim(f.raw_col_6), '') AS department_label_raw,
    COALESCE(NULLIF(btrim(f.raw_col_10), ''), NULLIF(btrim(f.raw_col_7), '')) AS category_label_raw,
    NULLIF(btrim(f.raw_col_5), '') AS source_stage_candidate,
    ...
    -- Parse / confidence / unresolved signals
    (f.parsed_business_date IS NULL) AS business_date_unresolved,
    ...
    (COALESCE(NULLIF(btrim(f.raw_col_10), ''), NULLIF(btrim(f.raw_col_7), '')) IS NULL AND NULLIF(btrim(f.raw_col_6), '') IS NULL) AS label_unresolved,
    (NULLIF(btrim(f.raw_col_5), '') IS NULL) AS shift_stage_unresolved,
    TRUE AS cost_nature_unresolved,
    (f.parsed_payment_channel IS NULL) AS payment_channel_unresolved,
    ...
    trim(both ';' from concat_ws('; ',
      CASE WHEN f.parsed_business_date IS NULL THEN 'business_date not safely parsed from column A' END,
      ...
      CASE WHEN COALESCE(NULLIF(btrim(f.raw_col_10), ''), NULLIF(btrim(f.raw_col_7), '')) IS NULL THEN 'category/detail (column G) missing/blank' END,
      CASE WHEN f.parsed_payment_channel IS NULL THEN 'payment_channel cannot be safely derived from column L' END
    )) AS unresolved_notes,

    CASE
      WHEN f.helper_total_amount_primary IS NOT NULL AND f.parsed_business_date IS NOT NULL THEN 'READY_FOR_ENRICHMENT'
  ...
  FROM v_eosq_fee_lines_raw_v1 f
